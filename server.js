...
function wipe_db(){
  // if ./.data/sqlite.db does not exist, wipe and recreate
  db.serialize(() => {
    if (exists) {
      db.run(
        'DROP TABLE IF EXISTS Users'
      );
      console.log("Table Users has DROP");
      create_db()
    }
  });
}

// POST request to wipe DB
app.post("/ClearUsers", (request, response) => {
  var clientSecret = request.headers['authorization']
...
app.post("/userTransfer", (request, response) => {
  ...
  console.log('Valid authorization');
  //console.log(request.body.data.events[0]['target'][0])
  console.log(request.body.data.events[0]['eventType'])
